import type {Component} from "solid-js";


const AddressCard: Component<{}> = props => {

    return (
        <div class="mx-5 mt-5">
            <div class="mt-4 flex flex-row items-start justify-between">
                <div>
                    <p class="text-sm font-bold text-gray-800">Brienno, Italy</p>
                    <p class="text-sm text-gray-800">49 kilometers away</p>
                    <p class="text-sm text-gray-800">Aug 18-25</p>
                    <p class="mt-2 text-sm text-gray-800"><strong>$2,135</strong> night</p>
                </div>
                <div class="flex flex-row items-center">
                    <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 20 20"
                         xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                    </svg>
                    <p class="text-sm">4,92</p>
                </div>
            </div>
        </div>
    )
}

export {AddressCard}
